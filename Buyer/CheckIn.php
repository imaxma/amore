...
require('../functions.php');
	...
	$querySQL="SELECT Id, Password FROM data_amore_admin WHERE name='".strtolower($_POST["username"])."' OR phone='".strtolower($_POST["username"])."' LIMIT 1";
	...
	if($result && $row = mysql_fetch_array($result)) {
		...
		if((md5($Password) === $_POST["password"]) || ($Password === $_POST["password"])) {
				...
				$_SESSION['curr_amore_admin_id'] = $row["Id"];

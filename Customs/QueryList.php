...
require('../functions.php');
	...
	//重要参数必须存在！
	if (!isset($LastItemID)) { exit(ReturnError(false, array("reason"=>"wrong parameter"))); };
	...
	if($LastItemID > 0)
		$querySQL = "SELECT * FROM data_customs WHERE ".$whereTmp." AND CustomsID=".$_SESSION['curr_customs_com_id']." AND ID<$LastItemID ORDER BY id DESC LIMIT $ItemCount";
	else 
		$querySQL = "SELECT * FROM data_customs WHERE ".$whereTmp." AND CustomsID=".$_SESSION['curr_customs_com_id']." ORDER BY id DESC LIMIT $ItemCount";
